...
var clearcookie = require('./controller/api/clearcookie');
...
var check_login = require('./controller/api/check_login');
...
const activate = require('./controller/activate');
...
const newpass = require('./controller/api/newpass');
router.get('/newpass', newpass);

router.get('/forgot', activate)

const storepass = require('./controller/api/storepass');
...
// searching grid 
const show = require('./controller/search_grid/show');

router.get('/search_grid', checkAuth, show);

//result
const result = require('./controller/result/result');
const marks = require('./controller/result/marks');
...
//delimeter search
const search = require('./controller/delimeter/search');
...
router.post('/job_application/update', checkAuth, server_val, update, (req, res) => {

  res.redirect(`/job_application?updated_id=${req.body.inserted_id}`)
})

router.post('/job_application/insert', checkAuth, server_val, insert, (req, res) => {
  let inserted_id = req.body.inserted_id
  res.redirect(`/job_application?inserted_id=${inserted_id}`)
...
const insert2 = require('./controller/job_appli2/api/insert');
...
const update2 = require('./controller/job_appli2/api/update');
