...
async function update_sql(arr, table) {
  ...
  if (table == "basic_detail") {
    ...
    let sql = `update schooling set board_name =?, passing_year = ?, percent = ? where appli_id = ? && class = ?`;
    ...
    if (data.result.affectedRows == 0) {
      let insert_arr = [arr[7], arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6]]
    ...
    let tech_delete = `delete from tech_kn where appli_id = ${arr[0]};`
