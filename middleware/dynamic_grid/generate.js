...
async function generate(req, res) {
  ...
  if (sql == '' || sql == undefined) {
    sql = ""
    data = ""
    res.render('dynamic_grid/home', { header: [], data: [], sql: sql, pathname: pathname, total: 1, query: req.query, current: 1, error: '' });
  }
    ...
    try {
      ...
      if ((sql.indexOf('update') == -1) && (sql.indexOf('drop') == -1) && (sql.indexOf('truncate') == -1) && (sql.indexOf('delete') == -1) && (sql.indexOf('alter') == -1) && (sql.indexOf('insert') == -1)) {
        ...
        res.render('dynamic_grid/home', { header: [], data: [], sql: '', pathname: pathname, current: 1, total: 1, query: req.query, error: "Invalid query" });
      }
