...
async function submit() {
  ...
  if (type == "update") {
    ...
    if (result.error != undefined) {
      current = 1;
      document.getElementById('try_err').innerHTML = result.error;
    }
    else {
      window.location.href = `/job_application2?updated_id=${result}`
    }
  }
  else if (type == "insert") {
    ...
    if (result.error != undefined) {
      ...
      window.location.href = `/job_application2?inserted_id=${result}`;
    }
  }
}